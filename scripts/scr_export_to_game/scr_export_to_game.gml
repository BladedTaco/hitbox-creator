...
//order the list items in the order needed for the game and iterate through
_list = obj_data.hurtbox_list
_list_name = "hurtbox_list"
repeat(2) { //hurtboxes, then hitboxes
	
	//sort the list
	k = array_length_1d(_list)
	for (l = 0; l < k; l++) { //for each bubble
		_table = true; //if the list is already sorted
		for (j = 0; j < k - l; j++) { //each character
			i = _list[j]
			o = _list[(j+1) mod k]
			if (i[0, 0] > o[0, 0]) { //if wrong order
				//switch the entries
				_list[j] = o
				_list[(j+1) mod k] = i
				_table = false;
...
backup = buffer_save_async(_buff, "Output_Buffer.buff", 0, buffer_tell(_buff)); //save to local backup this is copied later to destination
