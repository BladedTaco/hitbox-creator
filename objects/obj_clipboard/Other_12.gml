...
if (surface_exists(surface)) {
	...
	if (table[0, 0] + table[1, 0] != -20) {
		...
		hitbox = true
		for (i = 0; i < array_height_2d(table); i++) {
			if (table[i, 0] = -10) {
				...
				for (var o = 0; o < array_length_2d(table, i); o++) {
					if (o mod 2 = 0) {
						draw_set_colour(c_black)
					} else {
						draw_set_colour(c_white)
					}
					draw_text(16 + o*32, (i+1)*23, string(table[i, o]))
				}	
			}
		}
		...
		draw_text(width/2, 0, "Clipboard is empty.")
		hitbox = false
	}
